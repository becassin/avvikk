...
// Add Zen Tabs styles
if (theme_get_setting('avvikk_theme_tabs')) {
 ...
 *   Template variables.
 */
function avvikk_theme_preprocess_html(&$vars) {
  if (!module_exists('conditional_styles')) {
    avvikk_theme_add_conditional_styles();
  }
...
function avvikk_theme_preprocess_page(&$vars, $hook) {
...
function avvikk_theme_preprocess_node(&$vars) {
  // Add a striping class.
  $vars['classes_array'][] = 'node-' . $vars['zebra'];
}

function avvikk_theme_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function avvikk_theme_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('avvikk_theme_breadcrumb');
  if ($show_breadcrumb == 'yes' || ($show_breadcrumb == 'admin' && arg(0) == 'admin')) {



    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('avvikk_theme_breadcrumb_home');
    if (!$show_breadcrumb_home) {
      array_shift($breadcrumb);
    }
    // Return the breadcrumb with separators.
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('avvikk_theme_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('avvikk_theme_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('avvikk_theme_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
...
function avvikk_theme_id_safe($string) {
 ...
 */
function avvikk_theme_add_conditional_styles() {
...
function avvikk_theme_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = avvikk_theme_id_safe($element['#title']);
 ...
 */
function avvikk_theme_preprocess_menu_local_task(&$variables) {
...
function avvikk_theme_menu_local_tasks(&$variables) {
  $output = '';
